...
Files placed in this directory are automatically deployed to `~/` on the DNAnexus worker at job start, before `src/script.sh` runs.
...
It is called in `src/script.sh` as:
